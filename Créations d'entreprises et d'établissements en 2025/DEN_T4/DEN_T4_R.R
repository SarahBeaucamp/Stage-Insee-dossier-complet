# Nombre d'établissements économiquement actives en 2023
...
r <- dossier_complet %>%
  filter(GEO == "44109", ID_TAB == "DEN_T4") %>%
...
eco_act <- dossier_complet %>%
  ...
  mutate (
    secteur_act = case_when(
      ...
      TAB_MEASURE_LABEL == "Établissements – Activités immobilières" ~ "Activités immobilières",
      TAB_MEASURE_LABEL == "Établissements – Activités spécialisées, scientifiques et techniques et activités de services administratifs et de soutien" ~ "Activités spécialisées, scientifiques et techniques et activités de services administratifs et de soutien",
      TAB_MEASURE_LABEL == "Établissements – Construction" ~ "Construction",
      TAB_MEASURE_LABEL == "Établissements – Information et communication" ~ "Information et communication",
      ...
      TAB_MEASURE_LABEL == "Établissements – Commerce de gros et de détail, transports, hébergement et restauration" ~ "Commerce de gros et de détail, transports, hébergement et restauration",
      TAB_MEASURE_LABEL == "Établissements – Activités financières et d'assurance" ~ "Activités financières et d'assurance",
      TAB_MEASURE_LABEL == "Établissements – Autres activités de services" ~ "Autres activités de services"
    )
  ) %>%
...
ggplot(eco_act, aes(x = secteur_act, y = total_groupe)) +
  ...
  labs(
    title = "Nombre d'établissements économiquement actifs en 2023", 
    x = "",
    y = ""
  ) +
