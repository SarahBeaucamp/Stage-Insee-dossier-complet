---
title: "Analyse de la population française"
format: html
editor: visual
---

## Connexion à la base DuckDB et chargement des données
...
Le fichier contient **16 colonnes** :

- `GEO_REF` : Année\
- `GEO_OBJECT` : L'aire d'attraction des villes 2020 (AAV2020) est un ensemble de communes définissant l'influence d'un pôle de population et d'emploi sur les communes environnantes\
- `GEO_OBJECT_LABEL` : Aire d'attraction des villes 2020\
- `GEO` : Code département (ex : `001`)\
- `GEO_LABEL` : Ville (ex : `Paris`)\
- `TIME_PERIOD` : Année (ici 2026)\
- `ID_TAB` : Identifiant de table (ex : `TOU_T1`)\
- `ID_TAB_LABEL` : Libellé de la table (ex : *Nombre et capacité des hôtels au premier janvier*)\
- `DS` : Nom du dataset (ex : `DS_TOUR_CAP`)\
- `TAB_MEASURE` : ID de la variable mesurée (ex : `UNIT LOC ACTIVITY I551 UNIT LOC RANKING 3`)\
- `TAB_MEASURE_LABEL` : Libellé de la variable mesurée (ex : *Établissement, hôtels et hébergements similaires 3 étoiles*)\
- `OBS_VALUE` : Valeurs observées (entiers, minimum 90, maximum 170 920 sur les 10 premières lignes)\
- `OBS_STATUS` : Statut de l'observation (ex : `A`)\
- `UNIT_MULT` : NA sur les 10 premières lignes\
- `UNIT_MEASURE` : NA sur les 10 premières lignes\
- `CONF_STATUS` : NA sur les 10 premières lignes

------------------------------------------------------------------------

## Exploration des modalités de chaque variable
...
## Répartition par classe d'âge de la population française
...
### Diagramme en camembert pour la répartition de l'âge

```{r}
#| label: camembert-age
#| fig-cap: "Répartition de la population française par tranche d'âge (Recensement Insee 2022)"
...
## Répartition des différentes tranches d'âge par ville
...
structure_villes_propres
```

------------------------------------------------------------------------

## Répartition des différentes tranches d'âge par département
...
structure_departements
```

------------------------------------------------------------------------

## Répartition des différentes tranches d'âge par région

```{r}
#| label: age-par-region
