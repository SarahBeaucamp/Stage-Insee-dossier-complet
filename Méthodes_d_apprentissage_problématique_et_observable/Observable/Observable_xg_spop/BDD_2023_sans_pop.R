...
base_filtree_sans_pop_2023 <- dossier_complet_2023 %>%
  filter(
...
base_large_sans_pop_2023 <- base_filtree_sans_pop_2023 %>%
  group_by(GEO) %>%
...
names(base_large_sans_pop_2023) <- make.names(names(base_large_sans_pop_2023), unique = TRUE)
...
noms_sans_pop_2022_propres <- nettoyer_texte(noms_sans_pop_2022_ref)
noms_sans_pop_2023_propres <- nettoyer_texte(noms_sans_pop_2023_actuels)

# 5. Remplacement conditionnel : on écrase 2023 par le nom strict de 2022 si l'essence matche
for (i in seq_along(noms_sans_pop_2023_actuels)) {
  # On cherche à quelle position dans 2022 correspond le nom nettoyé de 2023
  index_correspondant <- match(noms_sans_pop_2023_propres[i], noms_sans_pop_2022_propres)
  
  if (!is.na(index_correspondant)) {
    # Si correspondance trouvée, on remplace le nom 2023 par le nom officiel 2022
    names(base_large_sans_pop_2023)[i] <- noms_sans_pop_2022_ref[index_correspondant]
  }
}
...
base_prete_sans_pop_2023 <- base_large_sans_pop_2023 %>%
  mutate(
...
summary(base_prete_sans_pop_2023$Y_GAP_ACT_GLOBAL)

# ------------------------------------------------------------------------------
# ÉTAPE 4 : FILTRE DE POPULATION ET TRAITEMENT HYBRIDE DES NAs
# ------------------------------------------------------------------------------

base_pre_filtre_sans_pop_2023 <- base_prete_sans_pop_2023 %>%
...
print(colonnes_secret_sans_pop_2023)

base_finale_sans_pop_2023 <- base_pre_filtre_sans_pop_2023 %>%
  mutate(across(-all_of(c(colonnes_secret_sans_pop_2023, "GEO", "GEO_LABEL")), ~ replace_na(., 0)))

compte_na_sans_pop_2023 <- colSums(is.na(base_finale_sans_pop_2023))
print("--- BILAN DES NAs APRÈS MISE À ZÉRO DES ÉQUIPEMENTS ---")
print(compte_na_sans_pop_2023[compte_na_sans_pop_2023 > 0])

referentiel_communes_sans_pop_2023 <- base_finale_sans_pop_2023 %>% select(GEO, GEO_LABEL)
...
base_finale_propre_sans_pop_2023 <- base_finale_sans_pop_2023 %>% 
